...
namespace RgbLed
{
    public sealed partial class SelectBulb : Page
    {
        ...
        private bool SprobujDodac(RgbLed.JedenDevice oNew)
        {
            ...
            System.Diagnostics.Debug.WriteLine("adding: " + oNew.sDisplayName);
        ...
        public async void fromDispatchShowItems()
        {
            if (bInside)
            {
                System.Diagnostics.Debug.WriteLine("czekam");
                for (int i = 1; i <= 10; i++)
                {
                    await Task.Delay(10);
                    if (!bInside)
            ...
            System.Diagnostics.Debug.WriteLine("nowa lista, count=" + RgbLed.App.moItemy.Count());
            uiListItems.ItemsSource = RgbLed.App.moItemy.GetList();
            bInside = false;
        }

        public async void toDispatch()
        {
            await uiListItems.Dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, this.fromDispatchShowItems);
        }

        private async void bt_Koniec(DeviceWatcher sender, object args)
        {
            // StopScan()
            await uiScan.Dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, this.StopScan);
        }

        private void bt_Added(DeviceWatcher sender, DeviceInformation args)
        {
            var oNew = new RgbLed.JedenDevice();
            System.Diagnostics.Debug.WriteLine("bt_Added, name=" + args.Name);
        ...
        private void Procesuje(bool bStart)
        {
            if (bStart)
            {
                ...
                if (dVal < 1) dVal = 100;   // ANDROID zabezpieczenie
