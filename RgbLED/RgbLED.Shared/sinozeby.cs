...
namespace RgbLed
{
    static class sinozeby
    {
        ...
        public async static Task<bool> IsNetBTavailable(bool bMsg)
        {
            ...
            bool bError = true;
            var oRadios = await Windows.Devices.Radios.Radio.GetRadiosAsync();
            ...
            {
                if (bMsg)
                    p.k.DialogBox("ERROR: no Bluetooth available");
                return false;
            }
            ...
            foreach (Windows.Devices.Radios.Radio oRadio in oRadios)
            {
                if (oRadio.Kind == Windows.Devices.Radios.RadioKind.Bluetooth & oRadio.State == Windows.Devices.Radios.RadioState.On)
                {
                    bError = false;
                    break;
                }
            ...
            {
                if (bMsg)
                    p.k.DialogBox("ERROR: Bluetooth is not enabled");
                return false;
            }
